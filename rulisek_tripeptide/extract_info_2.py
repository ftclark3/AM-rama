...
#directories = [f"{data_dir}/AWS", f"{data_dir}/DIR", f"{data_dir}/FHN", f"{data_dir}/MGT", f"{data_dir}/NKS", f"{data_dir}/PAT", f"{data_dir}/PED", f"{data_dir}/QWE", f"{data_dir}/TCR", f"{data_dir}/TWF", f"{data_dir}/WTS"] # for some reason, these didn't work the first time around. it seems that DIR caused issues, and the other ones failed cause of some weird multiprocessing thing stemming from the DIR problem
# I don't know why DIR was causing issues. it was some sort of issue with parsing the R residue, but nothing looks wrong with the pdb file. I'll just write a numpy array of all zeros for now as a workaround
directories = [f"{data_dir}/VPH"]
...
def process_all_confs_of_seq(directory):
    ...
    for filename in os.listdir(directory):
        ...
        with open(f"{directory}/{filename}",'r') as f, open(f"{directory}/also_bad.txt", 'w') as bad_log:
        ...
        st = parser.get_structure("",f"{directory}/{filename}") # first string is just a name, which we don't care about
        myChain = st[0]["X"] # it seems that all chains are named "X" in their PDB files
        ...
        if not expected_fail:
            raise SystemExit("weird residue thing")
        conf_info.append(r3.internal_coord.get_angle("phi"))
        conf_info.append(r3.internal_coord.get_angle("psi"))
...
"""
def inner_loop_interior(filename, bad):
    if filename in bad:
